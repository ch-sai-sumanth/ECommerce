 ...
 public static class DbExtension
    {
        public static IHost MigrateDatabase(this IHost host)
        {
            ...
            try
            {
                ...
                ApplyMigration(databaseSettings.ConnectionString,logger);
        ...
        private static void ApplyMigration(string connectionString, ILogger logger)
        {
            var retry = 5;
            while (retry > 0)
            {
                ...
                {
                    retry--;
                    logger.Error($"Migration attempt failed. Retries left: {retry}. Error: {ex.Message}");

                    if (retry == 0) throw;

                    // CRITICAL: Wait 2 seconds before trying again to let Postgres warm up
